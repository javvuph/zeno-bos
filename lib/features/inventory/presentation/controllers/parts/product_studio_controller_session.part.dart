...
extension ProductStudioControllerSession on ProductStudioController {
  ...
  void addImportReadyToCatalog() async { for (var item in importItems) { if (item.status == BulkScanStatus.ready) await repository.saveProduct(item.product.toDomain()); } notify(); }
  void updateImportItemField(int i, Function(ProductStudioData) f) { if (i < importItems.length) { f(importItems[i].product); notify(); } }
  void toggleImportSelectAll(bool v) { for (var item in importItems) item.isSelected = v; notify(); }
  void toggleImportSelectItem(int i, bool v) { if (i < importItems.length) { importItems[i].isSelected = v; notify(); } }
  ...
  void handleBulkBarcodeScanned(String b) async {
    ...
    if (existing != null) bulkScanItems.add(BulkScanItem(product: ProductStudioFromDomain.fromDomain(existing), status: BulkScanStatus.duplicate));
    else bulkScanItems.add(BulkScanItem(product: ProductStudioData.empty()..barcode = b, status: BulkScanStatus.ready));
    notify();
  }
  void addBulkReadyToCatalog() async { for (var item in bulkScanItems) { if (item.status == BulkScanStatus.ready) await repository.saveProduct(item.product.toDomain()); } notify(); }
  void updateBulkItemField(int i, Function(ProductStudioData) f) { if (i < bulkScanItems.length) { f(bulkScanItems[i].product); notify(); } }
  void toggleBulkSelectAll(bool v) { for (var item in bulkScanItems) item.isSelected = v; notify(); }
  void toggleBulkSelectItem(int i, bool v) { if (i < bulkScanItems.length) { bulkScanItems[i].isSelected = v; notify(); } }
