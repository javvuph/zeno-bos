...
extension ProductStudioControllerVariants on ProductStudioController {
  static int _barcodeSeqCounter = 0;
  ...
  void generateMatrix() {
    ...
    if (_product.barcode.trim().isNotEmpty) {
      usedBarcodes.add(_product.barcode.trim());
    }

    for (var color in selectedColors) {
      for (var size in selectedSizes) {
        final baseSku = _product.sku.trim();
        final defaultSku = _buildVariantSku(baseSku, color, size);
        final key = _variantKey(color, size);
        final existing = existingVariants[key];

        String variantSku = _normalizeVariantSku(existing?.sku, defaultSku);
        if (usedSkus.contains(variantSku)) {
          variantSku = '$defaultSku-${_product.variants.length + 1}';
        }
        usedSkus.add(variantSku);

        String barcode = (existing?.barcode.isNotEmpty == true) ? existing!.barcode : '';
        if (barcode.isEmpty || usedBarcodes.contains(barcode)) {
          do {
  ...
  void syncAllFromFirstRow() {
    ...
    for (int i = 1; i < _product.variants.length; i++) {
      ...
      v.price = first.price;
      v.purchasePrice = first.purchasePrice;
      v.mrp = first.mrp;
      v.stock = first.stock;
    }
    notify();
  ...
  void generateAllVariantBarcodes() {
    ...
    for (var v in _product.variants) {
      if (v.barcode.isNotEmpty) usedBarcodes.add(v.barcode);
    }
    for (var v in _product.variants) {
      if (v.barcode.isEmpty) {
        String newBc;
        do {
          newBc = _generateVariantBarcode();
        } while (usedBarcodes.contains(newBc));
        v.barcode = newBc;
        usedBarcodes.add(newBc);
      }
    }
    notify();
  }
  ...
  bool get hasVariants => selectedColors.isNotEmpty && selectedSizes.isNotEmpty;
  ...
  String _buildVariantSku(String baseSku, String color, String size) {
    final cleanStyle = baseSku.trim().replaceAll(RegExp(r'[^a-zA-Z0-9]'), '-').toUpperCase();
    final cleanColor = color.trim().replaceAll(RegExp(r'[^a-zA-Z0-9]'), '').toUpperCase();
    final cleanSize = size.trim().replaceAll(RegExp(r'[^a-zA-Z0-9]'), '').toUpperCase();

    if (cleanStyle.isEmpty) {
      return '$cleanColor-$cleanSize';
    }
    return '$cleanStyle-$cleanColor-$cleanSize';
  }
  ...
  String _generateVariantBarcode() {
    _barcodeSeqCounter = (_barcodeSeqCounter + 1) % 100000;
    final nowMicros = DateTime.now().microsecondsSinceEpoch;
    final combo = (nowMicros + _barcodeSeqCounter) % 10000000000;
    return '890${combo.toString().padLeft(10, '0')}';
  }
