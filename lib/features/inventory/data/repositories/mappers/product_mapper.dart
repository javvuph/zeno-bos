...
class ProductMapper {
  ...
  static ProductCollection mapToCollection(Product product, {ProductCollection? existing}) {
    final pc = (existing ?? ProductCollection())
      ..uuid = product.id..name = product.name..sku = product.sku.value..barcode = product.barcode?.value
      ..description = product.description ?? ''..unit = product.unit.name..categoryId = product.category?.id ?? ''
      ..brandId = product.brand?.id..basePrice = product.basePrice..taxRate = 0.0
