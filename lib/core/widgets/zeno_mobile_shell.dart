...
class _MobileBusinessSetup extends StatefulWidget {const _MobileBusinessSetup();@override State<_MobileBusinessSetup> createState()=>_MobileBusinessSetupState();}
class _MobileBusinessSetupState extends State<_MobileBusinessSetup>{final name=TextEditingController();final tax=TextEditingController();String country='India';String currency='INR';String language='English';bool taxEngine=true;bool security=true;@override void dispose(){name.dispose();tax.dispose();super.dispose();}@override Widget build(BuildContext context){final c=Theme.of(context).extension<ZenoSemanticColors>()!;return ListView(padding:const EdgeInsets.fromLTRB(16,10,16,110),children:[Text('BUSINESS SETUP',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900,color:c.textPrimary,letterSpacing:.5)),const SizedBox(height:4),Text('Configure your ZENO business workspace',style:TextStyle(fontSize:11,color:c.textSecondary)),const SizedBox(height:16),_setupCard(c,'01','BUSINESS IDENTITY',[_field(c,'Business / Store Name',name,Icons.storefront_outlined),_field(c,'GST / Tax ID',tax,Icons.receipt_long_outlined)]),_setupCard(c,'02','LOCALIZATION',[ _drop(c,'Country',country,['India','United States','United Arab Emirates'],(v)=>setState(()=>country=v)),_drop(c,'Currency',currency,['INR','USD','AED','EUR'],(v)=>setState(()=>currency=v)),_drop(c,'Language',language,['English','Malayalam','Arabic','Hindi'],(v)=>setState(()=>language=v))]),_setupCard(c,'03','TAX & SECURITY',[SwitchListTile(contentPadding:EdgeInsets.zero,title:Text('Tax Engine',style:TextStyle(fontSize:11,fontWeight:FontWeight.w800,color:c.textPrimary)),subtitle:Text('Apply configured tax rules to billing',style:TextStyle(fontSize:9,color:c.textSecondary)),value:taxEngine,onChanged:(v)=>setState(()=>taxEngine=v),activeThumbColor:c.accentPrimary),SwitchListTile(contentPadding:EdgeInsets.zero,title:Text('RBAC Security',style:TextStyle(fontSize:11,fontWeight:FontWeight.w800,color:c.textPrimary)),subtitle:Text('Role-based permissions enabled',style:TextStyle(fontSize:9,color:c.textSecondary)),value:security,onChanged:(v)=>setState(()=>security=v),activeThumbColor:c.accentPrimary)]),const SizedBox(height:8),SizedBox(height:52,child:ElevatedButton.icon(onPressed:()=>ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Business setup saved'))),icon:const Icon(Icons.check_circle_outline_rounded),label:const Text('SAVE & INITIALIZE BUSINESS',style:TextStyle(fontWeight:FontWeight.w900,letterSpacing:.7)),style:ElevatedButton.styleFrom(backgroundColor:c.accentPrimary,foregroundColor:Colors.black,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(15)))))]);}Widget _setupCard(ZenoSemanticColors c,String n,String title,List<Widget> children)=>Container(margin:const EdgeInsets.only(bottom:12),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:c.bgTier2,borderRadius:BorderRadius.circular(18),border:Border.all(color:c.borderSubtle)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Text(n,style:TextStyle(fontSize:9,fontWeight:FontWeight.w900,color:c.accentPrimary)),const SizedBox(width:8),Text(title,style:TextStyle(fontSize:11,fontWeight:FontWeight.w900,letterSpacing:1,color:c.textPrimary))]),const SizedBox(height:12),...children]);Widget _field(ZenoSemanticColors c,String label,TextEditingController ctl,IconData icon)=>Padding(padding:const EdgeInsets.only(bottom:10),child:TextField(controller:ctl,style:TextStyle(fontSize:11,color:c.textPrimary),decoration:InputDecoration(labelText:label,prefixIcon:Icon(icon,size:18),filled:true,fillColor:c.bgTier1,border:OutlineInputBorder(borderRadius:BorderRadius.circular(12),borderSide:BorderSide(color:c.borderSubtle)),enabledBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(12),borderSide:BorderSide(color:c.borderSubtle)))));Widget _drop(ZenoSemanticColors c,String label,String value,List<String> values,ValueChanged<String> onChanged)=>Padding(padding:const EdgeInsets.only(bottom:10),child:DropdownButtonFormField<String>(initialValue:value,items:values.map((v)=>DropdownMenuItem(value:v,child:Text(v,style:TextStyle(fontSize:11,color:c.textPrimary)))).toList(),onChanged:(v){if(v!=null)onChanged(v);},decoration:InputDecoration(labelText:label,filled:true,fillColor:c.bgTier1,border:OutlineInputBorder(borderRadius:BorderRadius.circular(12),borderSide:BorderSide(color:c.borderSubtle)),enabledBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(12),borderSide:BorderSide(color:c.borderSubtle)))));}
