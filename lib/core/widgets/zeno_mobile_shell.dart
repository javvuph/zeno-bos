...
class _MobileBusinessSetupState extends State<_MobileBusinessSetup> {
  int step=0; final data=_MobileStoreSetupData(); late final AdministrationController _admin;
  ...
  @override void initState(){super.initState(); _admin=AdministrationController(sl<IAdministrationRepository>());}
  @override void dispose(){_admin.dispose();super.dispose();}
  ...
  Future<void> _saveAndSync() async {
    ...
    await _admin.updateCompany(Company(id:old.id,name:data.storeName,taxId:data.taxId,gst:data.taxId,address:data.address,currency:data.currency,timezone:data.timezone,isDefault:old.isDefault));
