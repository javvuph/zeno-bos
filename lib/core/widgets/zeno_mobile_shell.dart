...
import 'package:zeno/features/procurement/presentation/controllers/procurement_controller.dart';
import 'package:zeno/features/procurement/domain/repositories/i_procurement_repository.dart';
...
class _MobileProcurementView extends StatefulWidget {
  final ValueChanged<String> onRoute;
  const _MobileProcurementView({required this.onRoute});
  @override State<_MobileProcurementView> createState()=>_MobileProcurementViewState();
}
class _MobileProcurementViewState extends State<_MobileProcurementView>{
  late final ProcurementController controller;
  @override void initState(){super.initState();controller=ProcurementController(sl<IProcurementRepository>());}
  @override void dispose(){controller.dispose();super.dispose();}
  @override Widget build(BuildContext context){
    final c=Theme.of(context).extension<ZenoSemanticColors>()!;
    return ListView(padding:const EdgeInsets.fromLTRB(14,10,14,110),children:[
      _hubHeader(c,'PROCUREMENT','PURCHASE • SUPPLIERS • RESTOCK',Icons.shopping_cart_checkout_rounded),
      const SizedBox(height:10),
      Row(children:[Expanded(child:_metric(c,'PURCHASE ORDERS',controller.purchaseOrders.length.toString(),Icons.receipt_long_rounded)),const SizedBox(width:8),Expanded(child:_metric(c,'SUPPLIERS',controller.suppliers.length.toString(),Icons.local_shipping_rounded))]),
      const SizedBox(height:12),
      Row(children:[Expanded(child:_action(c,'NEW PURCHASE',Icons.add_shopping_cart_rounded,()=>widget.onRoute('procurement/purchase-orders'))),const SizedBox(width:8),Expanded(child:_action(c,'SUPPLIERS',Icons.storefront_rounded,()=>widget.onRoute('procurement/suppliers')))]),
      const SizedBox(height:16),Text('PROCUREMENT WORKSPACE',style:TextStyle(fontSize:10,fontWeight:FontWeight.w900,letterSpacing:1.4,color:c.textSecondary)),const SizedBox(height:8),
      ...['Purchase Orders','Goods Receiving','Supplier Bills','Reorder Planning'].map((x)=>_workspaceTile(c,x,Icons.arrow_forward_ios_rounded,()=>widget.onRoute('procurement/purchase-orders')))
    ]);
  }
  Widget _hubHeader(ZenoSemanticColors c,String t,String s,IconData i)=>Container(padding:const EdgeInsets.all(17),decoration:BoxDecoration(gradient:ZenoTheme.aiGlowGradient,borderRadius:BorderRadius.circular(20)),child:Row(children:[Icon(i,color:Colors.black,size:30),const SizedBox(width:11),Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(t,style:const TextStyle(fontSize:16,fontWeight:FontWeight.w900,color:Colors.black)),Text(s,style:const TextStyle(fontSize:8,fontWeight:FontWeight.w800,color:Colors.black54,letterSpacing:1.1))])]));
  Widget _metric(ZenoSemanticColors c,String l,String v,IconData i)=>Container(padding:const EdgeInsets.all(11),decoration:BoxDecoration(color:c.bgTier2,borderRadius:BorderRadius.circular(15),border:Border.all(color:c.borderSubtle)),child:Column(children:[Icon(i,size:18,color:c.accentPrimary),const SizedBox(height:6),Text(v,style:TextStyle(fontSize:14,fontWeight:FontWeight.w900,color:c.textPrimary)),Text(l,style:TextStyle(fontSize:7,fontWeight:FontWeight.w800,color:c.textSecondary))]));
...
class _MobileModuleHub extends StatelessWidget {
  ...
  List<(String, IconData, String)> get actions {
    if (route.startsWith('orders/')) {
      return [
        ('Orders', Icons.shopping_bag_rounded, 'orders/dashboard'),
        ('Delivery', Icons.local_shipping_rounded, 'orders/dashboard'),
        ('New Order', Icons.add_circle_outline_rounded, 'orders/dashboard'),
      ];
    }
    if (route.startsWith('customers/')) {
      return [
        ('Customers', Icons.people_alt_rounded, 'customers/mgmt/list'),
        ('New Customer', Icons.person_add_alt_1_rounded, 'customers/mgmt/list'),
        ('CRM', Icons.insights_rounded, 'crm'),
      ];
    }
    if (route.startsWith('procurement/')) {
      return [
        ('Suppliers', Icons.local_shipping_outlined, 'suppliers'),
        ('Purchase', Icons.receipt_long_rounded, 'procurement'),
        ('Stock', Icons.inventory_2_outlined, 'inventory'),
      ];
    }
    if (route.startsWith('finance/')) {
      return [
        ('Finance', Icons.account_balance_wallet_rounded, 'finance'),
        ('Payments', Icons.payments_rounded, 'finance'),
        ('Reports', Icons.analytics_rounded, 'reports'),
      ];
    }
    if (route.startsWith('reports/')) {
      return [
        ('Analytics', Icons.insights_rounded, 'reports'),
        ('Sales', Icons.point_of_sale_rounded, 'sales/pos'),
        ('Finance', Icons.account_balance_wallet_rounded, 'finance'),
      ];
    }
    if (route.startsWith('ai/')) {
      return [
        ('AI Center', Icons.auto_awesome_rounded, 'ai/home'),
        ('Insights', Icons.insights_rounded, 'reports'),
        ('Inventory AI', Icons.inventory_2_rounded, 'inventory'),
      ];
    }
    if (route.startsWith('hr/')) {
      return [
        ('Staff', Icons.badge_rounded, 'staff'),
        ('Attendance', Icons.schedule_rounded, 'hr'),
        ('Roles', Icons.admin_panel_settings_rounded, 'admin'),
      ];
    }
    if (route.startsWith('admin/')) {
      return [
        ('Business Setup', Icons.store_rounded, 'admin/business-setup'),
        ('Security', Icons.security_rounded, 'admin'),
        ('Settings', Icons.settings_rounded, 'settings'),
      ];
    }
    if (route.startsWith('marketing/')) {
      return [
        ('Campaigns', Icons.campaign_rounded, 'marketing'),
        ('Customers', Icons.people_alt_rounded, 'customers/mgmt/list'),
        ('Analytics', Icons.insights_rounded, 'reports'),
      ];
    }
    if (route.startsWith('automation/')) {
      return [
        ('Workflows', Icons.account_tree_rounded, 'automation'),
        ('Alerts', Icons.notifications_active_rounded, 'notifications'),
        ('AI', Icons.auto_awesome_rounded, 'ai/home'),
      ];
    }
    if (route.startsWith('integrations/')) {
      return [
        ('Integrations', Icons.hub_rounded, 'integrations'),
        ('Payments', Icons.payments_rounded, 'finance'),
        ('Automation', Icons.bolt_rounded, 'automation'),
      ];
    }
    return [
      ('Dashboard', Icons.grid_view_rounded, 'dashboard'),
      ('Billing', Icons.point_of_sale_rounded, 'sales/pos'),
      ('Inventory', Icons.inventory_2_rounded, 'inventory'),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('MOBILE WORKSPACE • LIVE', style: TextStyle(fontSize: 8, fontWeight: FontWeight.w800, color: Colors.black.withValues(alpha: .65), letterSpacing: 1.1)),
                  ],
                ),
              ),
            ],
        ...
        const SizedBox(height: 12),
        Text('QUICK WORKSPACE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.5, color: c.textSecondary)),
        const SizedBox(height: 8),
        GridView.count(
          shrinkWrap: true,
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(x.$2, size: 23, color: c.accentPrimary),
                      const SizedBox(height: 9),
                      Text(x.$1, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: c.textPrimary)),
                      const SizedBox(height: 3),
                      Text('OPEN WORKSPACE', style: TextStyle(fontSize: 7, fontWeight: FontWeight.w700, color: c.textSecondary)),
                    ],
                  ),
                ),
              ),
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.auto_awesome_rounded, color: c.accentPrimary),
              const SizedBox(width: 10),
              Expanded(
                child: Text(
                  'ZENO keeps your mobile workflow connected to the same business data and navigation system.',
                  style: TextStyle(fontSize: 10, height: 1.45, color: c.textSecondary),
                ),
              ),
            ],
          ),
