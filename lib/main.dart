...
import 'package:zeno/app/zeno_responsive.dart';
...
Future<void> main() async {
  ...
  FlutterError.onError = (FlutterErrorDetails details) {
    ...
    final isSyncfusionDisposeBug =
        exceptionText.contains('RenderChartFadeTransition') ||
        exceptionText.contains('A disposed RenderObject was mutated') ||
        exceptionText.contains('disposed RenderObject');

    if (isSyncfusionDisposeBug) return;
...
class ZenoBOS extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => InventoryBloc(sl<AIProductService>())),
            ...
            home: LayoutBuilder(
              builder: (context, constraints) {
                final isMobile =
                    ...
                    ZenoResponsive.isMobile(context);
                if (isMobile) {
                  return ZenoMobileShell(
                    navigationController: NavigationController(),
                  );
                }
                return const ZenoShortcuts(child: ZenoShell());
              },
            ),
          );
        },
